...
class Pay extends Driver {
    ...
    protected function prepare($attributes = []){
        ...
        $attributes['appid'] = $this->extra['appid'];
        $attributes['mch_id'] = $this->extra['mch_id'];
        $attributes['spbill_create_ip'] = Yii::$app->request->userIP;
        $attributes['nonce_str'] = Yii::$app->security->generateRandomString(32);
        $attributes['sign'] = Util::makeSign($attributes,$this->extra['key']);
        ...
        try{
            ...
            Yii::error("创建订单异常:" . $e->getMessage());
            throw new Exception("预生成订单异常");
        }
    }
    ...
    public function checkSign($vals){
        $sign = Util::makeSign($vals,$this->extra['key']);
    ...
    public function nativeDefinedQrcode($productId){
        ...
        $sign = Util::makeSign($params,$this->extra['key']);
    ...
    public function nativeDefinedResponse($attributes){
        ...
        $responseParams['sign'] = Util::makeSign($responseParams,$this->extra['key']);
    ...
    /**
     * JSAPI支付
     * @param $attributes array JSAPI支付需要的参数
     * @return object
     */
    ...
    public function configForPayment($prepayId){
        ...
        try{
            $params['paySign'] = Util::makeSign($params,$this->extra['key']);

            return $params;
        }
        catch(Exception $e){
            Yii::error( "生成支付配置异常 : " . $e->getMessage() );
            throw new Exception( "生成支付配置异常 : " . $e->getMessage() );
        }
    ...
    protected function getNotify(){
        return (new Notify(['merchant'=>$this->extra]));
    }
    ...
    {
        $notify = (new RefundNotify(['merchant'=>$this->extra]))->getData();
